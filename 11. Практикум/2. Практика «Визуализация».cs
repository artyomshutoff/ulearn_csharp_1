...
public static class VisualizerTask
{
    ...
    public static Brush UnreachableAreaBrush =
      new SolidBrush(Color.FromArgb(255, 255, 230, 230));
    public static Brush ReachableAreaBrush =
      new SolidBrush(Color.FromArgb(255, 230, 255, 230));
    ...
    {
        PointF mathPoint =
          ConvertWindowToMath(new PointF(e.X, e.Y), GetShoulderPos(form));
        X = mathPoint.X;
        Y = mathPoint.Y;
        UpdateManipulator();
        form.Invalidate();
    ...
    public static void DrawManipulator(Graphics graphics,
                                       PointF shoulderPosition)
    {
        var joints =
        ...
        graphics.DrawString($"X={X:0}, Y={Y:0}, Alpha={Alpha:0.00}",
                            ...
                            10,
                            10);
        DrawReachableZone(graphics,
                          ReachableAreaBrush,
                          UnreachableAreaBrush,
                          shoulderPosition,
                          joints);
        for (var i = 0; i < joints.Length; i++) {
            ...
            graphics.FillEllipse(
              JointBrush, points[i].X - 5, points[i].Y - 5, 10, 10);
        }
    }
                                          ...
                                          PointF[] joints)
    {
        ...
        var mathCenter =
          new PointF(joints[2].X - joints[1].X, joints[2].Y - joints[1].Y);
    ...
    {
        return new PointF(form.ClientSize.Width / 2f,
                          form.ClientSize.Height / 2f);
    }

    public static PointF ConvertMathToWindow(PointF mathPoint,
                                             PointF shoulderPos)
    {
        return new PointF(mathPoint.X + shoulderPos.X,
                          shoulderPos.Y - mathPoint.Y);
    }

    public static PointF ConvertWindowToMath(PointF windowPoint,
                                             PointF shoulderPos)
    {
        return new PointF(windowPoint.X - shoulderPos.X,
                          shoulderPos.Y - windowPoint.Y);
    }
}
}