...
public class Terrain : ICreature
{
    ...
    public CreatureCommand Act(int x,
                               int y) => new CreatureCommand(){ DeltaX = 0,
                                                                DeltaY = 0 };
}
...
public class Player : ICreature
{
    ...
    public CreatureCommand Act(int x, int y)
    {
        ...
        if (!(x + dX >= 0 && x + dX < Game.MapWidth && y + dY >= 0 &&
              y + dY < Game.MapHeight))
            Stay();
        if (Game.Map[x + dX, y + dY] != null)
            if (Game
                  .Map [x + dX, y + dY]
                  .ToString() == "Digger.Sack")
                Stay();
...
public class Sack : ICreature
{
    ...
    {
        var result =
          new CreatureCommand{ DeltaX = 0, DeltaY = 1, TransformTo = this };
    ...
    private bool CanFallTo(int x, int y)
    {
        if (x < 0 || y < 0 || Game.MapWidth <= x || Game.MapHeight <= y)
            return false;
        return Game.Map.GetValue(x, y) == null ||
               IsFalling() && (Game.Map.GetValue(x, y) is Player ||
                               Game.Map.GetValue(x, y) is Monster);
    }
