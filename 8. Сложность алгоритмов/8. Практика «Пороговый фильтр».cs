...
{
    public static double[, ] ThresholdFilter(double[, ] original,
                                             double threshold)
    {
        ...
        for (var x = 0; x < width; x++)
            for (var y = 0; y < height; y++)
                filteredPixels[x, y] =
                  (original[x, y] >= thresholdValue) ? 1.0 : 0.0;

        return filteredPixels;
    }

    private static double CalculateThresholdValue(double[, ] original,
                                                  double threshold)
    {
        var width = original.GetLength(0);
        var height = original.GetLength(1);
    ...
    private static double GetThresholdValue(List<double> pixels,
                                            int numberOfPixels,
                                            int pixelsToChange)
    {
