...
public static class PathFinderTask
{
    public static int[] FindBestCheckpointsOrder(Point[] checkpoints)
    {
        ...
        MakePathPermutations(
          checkpoints, shortestPath, path, 1, shortestDistance);

        return shortestPath;
    }

    private static double MakePathPermutations(Point[] checkpoints,
                                               int[] shortestPath,
                                               int[] path,
                                               int position,
                                               double shortestDistance)
    {
        ...
        for (var i = 0; i < numberOfCheckpoints; i++) {
            ...
            if (index == -1) {
                path[position] = i;
                shortestDistance = MakePathPermutations(checkpoints,
                                                        shortestPath,
                                                        path,
                                                        position + 1,
                                                        shortestDistance);
